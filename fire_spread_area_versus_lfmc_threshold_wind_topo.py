...
#     ##### LFMC grad
    ...
    for angle in angles:
        ...
        lfmcLte = (lfmc<lfmcThresh[lc]).sum()
    ...
    toAppend = pd.DataFrame([[filename.split(".")[0], np.nansum(matrix[0]),np.nanmean(matrix[1])]+\
                             list(fireRanks*(np.array(fireRow)>0))+\
                                 ...
                                 list(elevationRanks*(np.array(elevationRow)>0))],\
                            columns = df.columns)
...
## size filter
df = masterdf.loc[masterdf.pixels>=100].copy() ## 1000 hectares
